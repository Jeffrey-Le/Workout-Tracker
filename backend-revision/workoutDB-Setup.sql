...
CREATE TABLE IF NOT EXISTS users (
    ...
    age INT CHECK (age >= 0),
    gender VARCHAR(10),
    height DECIMAL(6, 2) CHECK (height >= 0), -- increased precision
...
CREATE TABLE IF NOT EXISTS workouts (
    ...
    distance DECIMAL(6, 2) CHECK (distance >= 0), -- increased precision
...
CREATE TABLE IF NOT EXISTS bmi_history (
    ...
    user_id INT REFERENCES users(user_id) ON DELETE CASCADE,
    weight DECIMAL(6, 2) NOT NULL CHECK (weight >= 0), -- increased precision
    height DECIMAL(6, 2) NOT NULL CHECK (height >= 0), -- increased precision
    bmi DECIMAL(6, 2) GENERATED ALWAYS AS (weight / ((height / 100) ^ 2)) STORED, -- Automatically calculated with increased precision
